...
export function InstallGuide() {
  ...
  return (
    <div className="install-shell">
      <div
        className="install-tabs flex gap-2 overflow-x-auto border-b border-black/[0.06] p-2.5 dark:border-white/10"
        role="tablist"
      ...
      >
        {platformOptions.map(({ id, label, Icon }) => (
          <button
            ...
            className={`install-tab touch-target-y inline-flex min-w-max flex-1 items-center justify-center gap-2 rounded-2xl px-4 py-2.5 text-sm font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ios-blue focus-visible:ring-offset-2 dark:focus-visible:ring-offset-[#08111d] ${
              selectedPlatform === id
                ? 'install-tab--active'
      ...
      <AnimatePresence mode="wait" initial={false}>
        <motion.div
          ...
          className="install-panel grid gap-8 p-6 sm:p-8 lg:grid-cols-[1fr_auto] lg:items-center lg:p-10"
        >
          <div>
            <div className="install-illustration mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-ios-blue/10 text-ios-blue dark:bg-ios-blue/20 dark:text-blue-300">
            ...
            {installPrompt && selectedPlatform !== 'ios' && (
              <button
                ...
                className="premium-button premium-button--primary mt-6 min-h-12 px-5 py-3 text-base"
              >
          ...
          <ol className="grid gap-3 lg:w-[27rem]">
            {guide.steps.map((step, index) => (
              <li key={step} className="install-step flex items-start gap-3 rounded-2xl p-4 text-sm font-semibold leading-6 text-slate-700 dark:text-slate-200">
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-ios-blue text-xs font-black text-white shadow-sm shadow-blue-500/25">
                  {index + 1}
                </span>
                <span>{step}</span>
              </li>
